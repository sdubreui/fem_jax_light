...
if __name__ == '__main__':
    ...
    print("="*70)
    print("TEST DE L'ÉLÉMENT DKT OPTIMISÉ AVEC @JIT")
    print("="*70)
    
    # Configuration
    element_property = {'h': 0.01}
    material = {'E': 210e9, 'nu': 0.3}
    
    dkt_jax = DKT_element(element_property, material)
    ...
    print("\n--- Test de compute_K_elem ---")
    
    # Premier appel (compilation JIT)
    print("\n1. Premier appel (avec compilation JIT):")
    start = time.time()
    K_jax = dkt_jax.compute_K_elem(nodes)
    K_jax.block_until_ready()  # Attendre la fin du calcul
    time_first = time.time() - start
    print(f"   Temps: {time_first*1000:.2f} ms")
    print(f"   (Inclut la compilation JIT)")
    
    # Deuxième appel (code déjà compilé)
    print("\n2. Deuxième appel (code compilé):")
    start = time.time()
    K_jax2 = dkt_jax.compute_K_elem(nodes)
    K_jax2.block_until_ready()
    time_second = time.time() - start
    print(f"   Temps: {time_second*1000:.2f} ms")
    print(f"   Accélération vs 1er appel: {time_first/time_second:.1f}×")


    from tri_elements import DKT_element as DKT_numpy
    import numpy as np    
    dkt_numpy = DKT_numpy(element_property, material)
    nodes_numpy = np.array(nodes)
    start = time.time()
    K_numpy = dkt_numpy.compute_K_elem(nodes_numpy)
    end = time.time()

    print(f"Matrice K calculée en {(end-start)*1000:.2f} ms")
    error = np.linalg.norm(np.array(K_jax) - K_numpy) / np.linalg.norm(K_numpy)
    print(f"Erreur relative avec version NumPy: {error:.2e}")
    
    if error < 1e-10:
        print("✓ VALIDATION RÉUSSIE: Les deux implémentations concordent!")
    else:
        print("⚠ Attention: Différence détectée entre JAX et NumPy")


    #Calcul des déformations et contraintes
    print("\n--- Test de compute_strain_and_stress ---")
    U_elem = jnp.zeros(18) 
    U_elem = U_elem.at[0].set(1.0)  # Déplacement en x du nœud 1 => test membrane
    U_elem = U_elem.at[2].set(1.0)  # Déplacement en z du nœud 1 => test flexion
    strains, stresses, coords = dkt_jax.compute_strain_and_stress(nodes, U_elem,z=0.01/2)
    end = time.time()
    print(f"Déformations et contraintes calculées en {(end-start)*1000:.2f} ms")
    print("Déformations aux points de Gauss:")
    print(strains)
    print("Contraintes aux points de Gauss:")
    print(stresses) 
    print("Coordonnées des points de Gauss (globales):")
    print(coords)
    
    #comparaison avec NumPy
    strains_numpy, stresses_numpy,pts_gauss = dkt_numpy.compute_strain_and_stress(nodes_numpy, U_elem)
