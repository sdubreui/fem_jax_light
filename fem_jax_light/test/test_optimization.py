...
def obj_fun(X):
    ...
    # reshape pour regrouper par sections
    new_nodes_sorted_reshaped = new_nodes_sorted.reshape(y_sections.shape[0], nx, -1)

    dx = new_nodes_sorted_reshaped[:, :, 1]  # coordonnée x pour chaque section

    # appliquer la transformation vectorisée
    DY = dx + (-DT[:, None] * dx + DT[:, None])

    # remettre dans la structure
    new_nodes_sorted_reshaped = new_nodes_sorted_reshaped.at[:, :, 1].set(DY)

    # revenir au format initial
    new_nodes_sorted = new_nodes_sorted_reshaped.reshape(-1, new_nodes.shape[1])
...
def h(X):
    ...
    K = fem.assembling_K_parametric(new_nodes[:,1:], element_property, material)
    # Set the rhs
    rhs = fem.set_rhs(F)
...
#constraint using the sparse solver to compute the maximum deflection at the top edge of the beam
def h_sparse(X):
    # X is the vector that parameterized the shape of the beam thickness 
    y_sections_norm = jnp.array(y_sections/y_sections.max())
    ind_y = jnp.argsort(y_sections_norm)
    ...
    new_nodes = new_nodes.at[ind_y].set(new_nodes_sorted)
    # # Assembling the rigidity matrix
    K_sp = fem.assembling_K_parametric_sparse(new_nodes[:,1:], element_property, material)
    # Set the rhs
    rhs = fem.set_rhs(F)
    # Apply boundary conditions
    K_sp, rhs = fem.boundary_conditions_sparse(K_sp, rhs)
    # solve 
    Us = fem.solve_sparse(K_sp,rhs)
    c = Us[0::6].min()+1.5
    return (c)
